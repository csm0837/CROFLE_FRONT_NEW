...
<template>
    <div class="recovery-container">
      <div v-if="!isTokenValid" class="recovery-box">
        <div class="error-message text-center">
          ...
          <p class="mb-4">비밀번호 재설정 링크가 만료되었거나 유효하지 않습니다.</p>
      ...
      <div v-else-if="!isCompleted" class="recovery-box">
        ...
        <div v-if="status.message" 
          :class="['status-message', status.type === 'error' ? 'error' : 'success']">
          {{ status.message }}
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      async checkToken() {
        try {
          const response = await axios.get(`/api/account/reset-password/validate`, {
            params: {
              token: this.token,
              email: this.email
            }
          })
          this.isTokenValid = response.status === 200
        } catch (error) {
      ...
      async handleSubmit() {
        try {
          ...
          const response = await axios.post('/api/account/reset-password/confirm', {
            token: this.token,
            email: this.email,
          ...
          this.status.message = '비밀번호 변경에 실패했습니다. 다시 시도해주세요.'
        } finally {
